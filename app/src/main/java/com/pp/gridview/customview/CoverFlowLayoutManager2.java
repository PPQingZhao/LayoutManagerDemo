...
public class CoverFlowLayoutManager2 extends RecyclerView.LayoutManager {
    ...
    public synchronized int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        //这里不需要调用offsetChildrenVertical()　会在recyclerViewFillView里面对itme重新布局
//        //开始移动
...
//        可见区域出现在屏幕上的子view
    ...
    private Rect getRecyclerViewFrame() {
        ...
        Rect recyclerViewFrame = null;
        if (orientation == VERTICAL) {
            recyclerViewFrame = new Rect(0, 0 + verticalScrollOffset, recycleViewWidth, recyclerViewHeight + verticalScrollOffset);
        } else {
            recyclerViewFrame = new Rect(0, 0, recycleViewWidth, recyclerViewHeight);
        }
    ...
    public int calucRowInPager(int index) {
        return index % (columCount * rowCount) / rowCount;
    }
